...
import { pool } from "./db.js"; // conexión real a Postgres
...
// GET /products/with-users (consultar DB + llamar users-api)
app.get("/products/with-users", async (_req, res) => {
  try {
    ...
    res.json({
      ...
      users: Array.isArray(users) ? users : [],  // listado de usuarios
...
// GET /products (todos los productos desde DB)
app.get("/products", async (_req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "query failed", detail: String(e) });
  }
});
...
// POST /products (crear en la DB)
app.post("/products", async (req, res) => {
  const { id, name, price } = req.body ?? {};
  if (!id || !name || !price)
    return res
      .status(400)
      .json({ error: "id, name & price are required" });
  ...
  try {
    ...
    res
      .status(500)
      .json({ error: "insert failed", detail: String(e) });
  }
});

// PUT /products/:id (actualizar en la DB)
app.put("/products/:id", async (req, res) => {
  const { name, price } = req.body ?? {};
  if (!name || !price)
    return res
      .status(400)
      .json({ error: "name & price required" });
  ...
  try {
    ...
    res
      .status(500)
      .json({ error: "update failed", detail: String(e) });
  }
});

// DELETE /products/:id (eliminar de la DB)
app.delete("/products/:id", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "delete failed", detail: String(e) });
  }
});
